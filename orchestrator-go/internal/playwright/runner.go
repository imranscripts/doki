...
func (m *Manager) resolveProject(projectID string) (string, string, error) {
	...
	playwrightRoot := filepath.Join(m.appDir, "playwright")

	if projectID == "" || projectID == "_default" {
		if !isDir(playwrightRoot) {
			return "", "", fmt.Errorf("project directory not found: %s", playwrightRoot)
		}
		return playwrightRoot, "default", nil
	}
	...
	projectDir := filepath.Join(playwrightRoot, projectID)
	if !isDir(projectDir) {
...
func resolveTestSelection(projectDir string, requested []string) ([]string, int, error) {
	if len(requested) > 0 {
		...
		for _, raw := range requested {
			...
			containerPath := name
			if !strings.HasPrefix(containerPath, "tests/") {
				containerPath = filepath.ToSlash(filepath.Join("tests", containerPath))
			}
	...
	searchDir := filepath.Join(projectDir, "tests")
	if !isDir(searchDir) {
	...
	for _, pattern := range patterns {
		matches, _ := filepath.Glob(filepath.Join(searchDir, pattern))
		count += len(matches)
	}
	if count == 0 {
...
func writeWrapperConfig(projectDir string, settings wrapperSettings) (string, func(), error) {
	...
	original := ""
	switch {
	case fileExists(filepath.Join(projectDir, "playwright.config.ts")):
		original = "playwright.config.ts"
	case fileExists(filepath.Join(projectDir, "playwright.config.js")):
		original = "playwright.config.js"
	}
