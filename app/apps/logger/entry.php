...
$entry = $identifier !== '' ? $manager->getEntry($identifier) : null;

function logger_entry_e($value): string {
...
<html lang="en">
<head>
    <?php $layout->renderHead($entry ? 'Logger - ' . $entry['name'] : 'Logger Entry'); ?>
    <style>
        .logger-page {
            display: grid;
            gap: 18px;
...
<body>
            ...
            <div class="content-area">
                ...
                <?php if ($entry === null): ?>
                    <div class="logger-empty">
                        ...
                        <div>Logger entry not found</div>
                    ...
                    <div class="logger-page">
                        <section class="logger-panel">
                            <div class="logger-panel-header">
                                <h2 class="logger-panel-title"><?= logger_entry_e($entry['name']) ?></h2>
                                <div class="logger-pills">
                                    <span class="logger-pill"><i class="fas fa-route"></i><?= logger_entry_e($entry['mode']) ?></span>
                                    <span class="logger-pill"><i class="fas fa-layer-group"></i><?= (int)$entry['source_count'] ?> sources</span>
                                    <span class="logger-pill"><i class="fas fa-filter"></i><?= (int)$entry['filter_count'] ?> filters</span>
                                    <?php if ($isAdmin): ?>
                                    <span class="logger-pill"><i class="fas fa-satellite-dish"></i>polling</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="logger-panel-body">
                                <?php if ($entry['sources'] === []): ?>
                                    <div class="logger-empty">No sources configured</div>
                                <?php else: ?>
                                    <div class="logger-source-list">
                                        <?php foreach ($entry['sources'] as $source): ?>
                                        <?php $resolution = $isAdmin ? $resolver->resolveSourceTarget($entry, $source) : null; ?>
                                        <div class="logger-source">
                                            <div class="logger-source-name">
                                                <i class="fas fa-file-lines"></i>
                                                <?= logger_entry_e($source['source_name']) ?>
                                            </div>
                                            <div class="logger-path"><?= logger_entry_e($source['path']) ?></div>
                                            <?php if ($isAdmin): ?>
                                            <div class="logger-pills">
                                                <?php if (!empty($source['target_id'])): ?>
                                                <span class="logger-pill"><i class="fas fa-server"></i><?= logger_entry_e($source['target_id']) ?></span>
                                                <?php endif; ?>
                                                <?php if (!empty($source['environment_target_key'])): ?>
                                                <span class="logger-pill"><i class="fas fa-layer-group"></i><?= logger_entry_e($source['environment_target_key']) ?></span>
                                                <?php endif; ?>
                                                <?php if (!empty($source['target_type'])): ?>
                                                <span class="logger-pill"><i class="fas fa-plug"></i><?= logger_entry_e($source['target_type']) ?></span>
                                                <?php endif; ?>
                                                <?php if (is_array($resolution) && $resolution['success']): ?>
                                                <span class="logger-pill"><i class="fas fa-check"></i>resolves to <?= logger_entry_e($resolution['target']['id']) ?></span>
                                                <?php elseif (is_array($resolution)): ?>
                                                <span class="logger-pill"><i class="fas fa-triangle-exclamation"></i><?= logger_entry_e($resolution['error']) ?></span>
                                                <?php endif; ?>
                                            </div>
                                            <?php endif; ?>
                                        </div>
                        ...
                        <section class="logger-panel">
                            <div class="logger-panel-header">
                                <h2 class="logger-panel-title">Log Output</h2>
                                <div class="logger-pills">
                            ...
                            <div class="logger-panel-body">
                                <?php if (!empty($entry['filters'])): ?>
                                <div class="saved-filters" id="savedFilters">
                                    <?php foreach ($entry['filters'] as $filter): ?>
                                    <button class="filter-chip" type="button" data-filter-id="<?= logger_entry_e($filter['id']) ?>" <?= $filter['enabled'] ? '' : 'disabled' ?>>
                                        <i class="fas fa-filter"></i>
                                        <?= logger_entry_e($filter['name']) ?>
                                    </button>
                                    <?php endforeach; ?>
    ...
    <?php if ($entry !== null): ?>
    <script>
        const loggerEntry = <?= json_encode([
            'id' => $entry['id'],
            'slug' => $entry['slug'],
            'name' => $entry['name'],
        ], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
        const loggerFilters = <?= json_encode($entry['filters'] ?? [], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
        ...
        function lineHaystack(line) {
            return `${line.sourceName || ''} ${line.path || ''} ${line.line || ''}`;
        }
        ...
        function filterMatchesLine(filter, line) {
            ...
            const haystackRaw = `${line.sourceName || ''} ${line.path || ''} ${line.line || ''}`;
        ...
        function renderSourceHealth(sources = []) {
            ...
            sourceHealth.innerHTML = sources.map((source) => {
                ...
                const title = hasError
                    ? `${source.name || 'source'}: ${source.error}`
                ...
                return `
                    <div class="source-health-item ${hasError ? 'error' : ''}" title="${escapeHtml(title)}">
                        ...
                        <span class="source-health-name">${escapeHtml(source.name || 'source')}</span>
            ...
            loggerConsole.innerHTML = visible.map((line) => {
                ...
                return `
                    <div class="log-row" data-line-id="${escapeHtml(line.id)}">
                        <div class="log-prefix" title="${escapeHtml(line.path)}">${escapeHtml(line.sourceName)}${lineNumber}</div>
        ...
        async function fetchLatest(replace = true) {
            ...
            try {
                ...
                if (!data.success) {
                    setStatus(data.error || 'Tail failed.', 'error');
                    return;
                }
                ...
                renderSourceHealth([]);
                setStatus(`Tail failed: ${error.message}`, 'error');
            }
        }
